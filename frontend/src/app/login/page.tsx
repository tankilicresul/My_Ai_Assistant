...
function LoginForm() {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-[#F8F9FB] relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[350px] bg-orange-400/10 blur-[130px] pointer-events-none rounded-full" />
      <div className="absolute bottom-0 right-10 w-[400px] h-[300px] bg-amber-400/10 blur-[100px] pointer-events-none rounded-full" />
      ...
      <header className="px-6 py-6 border-b border-slate-200 bg-white/80 backdrop-blur-md flex items-center justify-between max-w-7xl w-full mx-auto">
        ...
        >
          Hesabınız yok mu? <span className="text-orange-600 font-bold underline underline-offset-4">Kayıt Olun</span>
        </Link>
      </header>
      ...
      <footer className="py-6 text-center text-xs text-slate-500 bg-white border-t border-slate-200">
        TanCoreLab Güvenli Kimlik Doğrulama Sistemi • 256-bit SSL & JWT Korumalı
      </footer>
    </div>
  );
}
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#F8F9FB] flex items-center justify-center text-slate-500 text-sm">Yükleniyor...</div>}>
      <LoginForm />
    </Suspense>
  );
}
